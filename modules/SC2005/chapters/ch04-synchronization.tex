...
Many threads may \emph{read} shared data at once, but a \emph{writer} needs exclusive access. Intuition: readers share a library; a writer needs the whole room. The first-readers solution guards a \texttt{readcount} with a \texttt{mutex}: the first reader locks \texttt{wrt} (blocking writers), the last reader unlocks it, and writers simply lock \texttt{wrt}. Readers never block each other, so concurrent reads stay fast. The flaw: a steady stream of readers can starve a waiting writer forever---the reader--writer exercise below asks you to flip the priority (at the cost of possible reader starvation). Real systems (Linux \texttt{rwsem}, databases) add fairness queues instead.
