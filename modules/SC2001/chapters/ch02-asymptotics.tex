...
\begin{enumerate}[label=(L\arabic*)]
    ...
    \item analyse amortised cost over an operation sequence (aggregate, accounting, potential);
...
Two programs solve the same task: $A$ takes $f_{A}(n)=3n^{2}+5n$~steps, $B$ takes $f_{B}(n)=100n$~steps (constant factors from hardware). For small $n$, $B$ may be slower; for large $n$, shape wins and $B$ crushes $A$ regardless of the $100\times$ constant. ``Large $n$'' means $\exists n_{0}\,\forall n\ge n_{0}$.

\begin{table}[h]\centering\small
\begin{tabular}{r r r l}
\toprule $n$ & $3n^{2}+5n$ & $100n$ & winner \\\midrule
10 & 350 & 1\,000 & $A$ (350 $<$ 1\,000) \\
100 & 30\,500 & 10\,000 & $B$ \\
1\,000 & 3\,005\,000 & 100\,000 & $B$ by $30\times$ \\
10\,000 & 300\,050\,000 & 1\,000\,000 & $B$ by $300\times$ \\\bottomrule
\end{tabular}
\caption{Shape overtakes constants: quadratic $3n^{2}+5n$ beats $100n$ only below the crossover $n_{0}\approx33$; after that the linear algorithm wins forever.}
\label{tab:shape-wins}
\end{table}

We consider $f,g:\mathbb{N}\to\mathbb{R}_{\ge0}$ eventually non-negative; ``after $n_{0}$'' is the formal handle for ``for large $n$.''

% ============================================================
\section{The Five Notations}
\label{sec:five-notations}
\textbf{Picture first.} $f=O(g)$ means ``$f$ stays under a scaled copy of $g$ after some cutoff $n_{0}$'' (\emph{one} suitable $c$ suffices). $\Omega$ is the mirror (stays above), $\Theta$ is sandwiched between two scalings, while $o$/$\omega$ require the sandwich for \emph{every} $c>0$ --- the band can be made arbitrarily tight (dashed narrowing bands in Fig.~\ref{fig:asymptotic-bands}). The formal $\exists c\,\exists n_{0}\,\forall n\ge n_{0}$ below is just that picture in quantifiers.
...
\begin{tikzpicture}[scale=0.70, every node/.style={font=\scriptsize}]
  ...
  \begin{scope}
    ...
    \draw[red, dashed, thin, domain=0.15:\w, smooth] plot (\x,{0.24*\x*\x+0.45});
    \node[red, font=\tiny] at (2.4,2.00) {$c'g$};
    \fill[blue!12] (1.0,0.55) -- plot[domain=1.0:\w] (\x,{0.32*\x*\x+0.65}) -- (\w,0.55) -- cycle;
    \node at (1.9,-0.55) {$f=O(g)$};
    \node[font=\tiny, blue!60!black] at (1.9,1.9) {$f\le c\,g$};
    \node[font=\tiny, red!60!black] at (1.9,1.05) {$o$: every $c$};
  \end{scope}
  % --- Omega panel ---
  \begin{scope}[xshift=5.0cm]
    ...
    \draw[red, dashed, thin, domain=0.15:\w, smooth] plot (\x,{0.10*\x*\x+0.10});
    \fill[green!12] (1.0,0.50) -- plot[domain=1.0:\w] (\x,{0.18*\x*\x+0.18}) -- plot[domain=\w:1.0] (\x,{0.30*\x*\x+0.55}) -- cycle;
    \node at (1.9,-0.55) {$f=\Omega(g)$};
    \node[font=\tiny, red!60!black] at (1.9,0.35) {$\omega$: every $c$};
  \end{scope}
  % --- Theta panel ---
  \begin{scope}[xshift=10.0cm]
    ...
    \fill[orange!14] (1.0,0.22) -- plot[domain=1.0:\w] (\x,{0.13*\x*\x+0.05}) -- plot[domain=\w:1.0] (\x,{0.35*\x*\x+0.80}) -- cycle;
    \node at (1.9,-0.55) {$f=\Theta(g)$};
  \end{scope}
\end{tikzpicture}
\caption{$O$, $\Omega$, $\Theta$ as eventually-sandwiching bands (color fills). Dashed red curves show $o$/$\omega$: the sandwich must hold for \emph{every} $c>0$, so the band can be narrowed arbitrarily. Only $n\ge n_{0}$ matters.}
...
\noindent\fbox{\parbox{0.97\linewidth}{\small\textbf{Intuition (no calculus needed).} If the ratio $f(n)/g(n)$ settles near a constant, $f$ and $g$ grow at the same rate ($\Theta$); if it tends to $0$, $f$ is negligible ($o$); if it blows up, $f$ dominates ($\omega$). Numbers first: for $f(n)=3n^{2}+5n$, $g(n)=n^{2}$,
\begin{center}\small\begin{tabular}{r r r}\toprule $n$ & $f(n)/g(n)=(3n^{2}+5n)/n^{2}$ & $\to$ \\\midrule 10 & 3.50 & \\ 100 & 3.05 & 3 \\ 1\,000 & 3.005 & \\ \bottomrule\end{tabular}\end{center}
The ratio $\to3\in(0,\infty)$, so we expect $f=\Theta(g)$. The theorem below makes this precise; l'H\^opital is just an optional calculus shortcut for the limit, not the definition.}}
\smallskip
...
\end{example}
\emph{Optional shortcut:} l'H\^opital for $\infty/\infty$, and $\log_{a}n=\Theta(\log_{b}n)$ for any $a,b>1$ (change of base is a constant factor). Hence we write simply $\Theta(\log n)$.
...
\begin{tikzpicture}[scale=0.78, every node/.style={font=\scriptsize}]
  ...
  \fill[teal!5] (0,0) rectangle (8.8,1.1);
  \fill[orange!6] (0,1.1) rectangle (8.8,2.2);
  \fill[blue!5] (0,2.2) rectangle (8.8,3.3);
  \fill[red!4] (0,3.3) rectangle (8.8,4.6);
...
\label{sec:cases}
\noindent\fbox{\parbox{0.97\linewidth}{\small\textbf{Micro-primer: expectation.} Fix $N$ inputs of size $n$ and pick one uniformly at random (each with prob.\ $1/N$). The \emph{average} running time is the ordinary average $\mathbb{E}[T]=\frac1N\sum_{i=1}^{N}T(I_{i})$. For an event, the \emph{indicator} $X=\mathbf{1}[\text{event}]$ is $1$ if it happens, $0$ otherwise, so $\mathbb{E}[X]=\Pr[\text{event}]$ and $\mathbb{E}[\sum X_{j}]=\sum\mathbb{E}[X_{j}]$ (linearity).}}
\smallskip
...
Average-case analysis needs the distribution $D_{n}$ stated explicitly. Randomised algorithms (Ch.~8) instead randomise the \emph{algorithm} and bound $\mathbb{E}[T(I)]$ for every $I$.

% ============================================================
\section{Amortised Analysis: Averaging Over a Sequence}
\label{sec:amortised}
\emph{Intuition:} worst case asks ``how bad can \emph{one} operation be?'' Amortised analysis asks ``how bad can $n$ operations be \emph{in total}, divided by $n$?'' --- an average over the \emph{sequence}, with no randomness and no input distribution (unlike average-case, Sec.~\ref{sec:cases}). Expensive steps are rare by structure: most operations are cheap and prepay the costly ones. Three views of the same idea: \emph{aggregate} (bound the total directly), \emph{accounting} (overcharge cheap ops; banked credit pays for dear ones), \emph{potential} (store savings in $\Phi(\text{state})$; charge $\text{amortised}=\text{actual}+\Delta\Phi$).
\begin{example}[Dynamic-table doubling] Start capacity $1$, double when full. Copies across $n$ appends total $1+2+4+\cdots+2^{\lfloor\log_2(n-1)\rfloor}<2n$: total $O(n)$, so each append is $O(1)$ \emph{amortised} though one append costs $\Theta(n)$. Accounting view: charge $3$ per append ($1$ spent, $2$ banked) --- the bank always covers the next doubling. Potential view: $\Phi=2\cdot\text{size}-\text{capacity}\ge0$ releases exactly the doubling cost. Contrast with average-case: no distribution is assumed; the bound holds for \emph{every} sequence.
\end{example}
