...
\begin{tikzpicture}[every node/.style={circle,draw,inner sep=2pt,font=\small},
  level distance=12mm, sibling distance=14mm,
  edge from parent/.style={draw,-stealth}]
\node {55}
  child { node {25} child { node {12} } child { node {13} } }
  child { node {30} child { node {14} } child { node {16} } };
\end{tikzpicture}
...
\begin{figure}[h]\centering
\begin{tikzpicture}[scale=0.95, every node/.style={circle,draw,inner sep=2pt,font=\small}]
  \node (a) at (0,1.5) {$a$}; \node (b) at (2,1.5) {$b$};
  \node (c) at (0,0) {$c$}; \node (d) at (2,0) {$d$};
  \draw[dashed,rounded corners] (-0.5,-0.5) rectangle (1,2);
