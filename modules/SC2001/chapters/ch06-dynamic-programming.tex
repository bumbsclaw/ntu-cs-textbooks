...
Picture a grid with $X$ down the rows and $Y$ across the columns. The \emph{longest zigzag} from $(0,0)$ to $(m,n)$---step $\diagup$ on a match, otherwise $\uparrow$ or $\leftarrow$---collects the most matching characters; its length \emph{is} the LCS.

\begin{center}
\begin{tikzpicture}[scale=0.85,every node/.style={font=\scriptsize}]
\tikzset{cell/.style={draw,minimum size=7mm,anchor=center}}
\foreach \x in {0,...,3}\foreach \y in {0,...,3}\node[cell,fill=gray!5] at (\x*0.8,-\y*0.8) {};
\node[font=\tiny] at (1.2,0.45) {$Y_j$}; \node[font=\tiny] at (-0.65,-1.2) {$X_i$};
\draw[->,blue,thick] (0.8,-0.8) -- (1.6,-1.6) node[midway,above,sloped,font=\tiny] {$\diagup$ if $x_i{=}y_j$};
\draw[->,orange,thick] (1.6,-0.8) -- (1.6,-1.6) node[midway,right,font=\tiny] {$\uparrow$};
\draw[->,violet,thick] (0.8,-1.6) -- (1.6,-1.6) node[midway,below,font=\tiny] {$\leftarrow$};
\node[font=\tiny,below] at (1.2,-3.0) {empty $c$ table: each cell depends on 3 neighbours; longest zigzag = LCS};
\end{tikzpicture}
\end{center}
...
\begin{center}\begin{tikzpicture}[every node/.style={font=\scriptsize}]
\tikzset{c/.style={draw,minimum width=6mm,minimum height=5mm,anchor=center}}
\foreach \i in {0,...,5}\node[c,fill=blue!8] at (\i*0.75,0) {\i};
\draw[->,red,thick] (3.8,0.42) -- (0.3,0.42) node[midway,above,font=\tiny] {backwards: $dp[w]\!\leftarrow\!dp[w{-}w_i]$ (old row only)};
\node[font=\tiny,below] at (1.9,-0.35) {1-D $dp[0..W]$ --- forward loop reuses item (unbounded); backward prevents reuse};
...
\begin{center}\begin{tikzpicture}[every node/.style={font=\scriptsize},level distance=10mm,sibling distance=18mm,box/.style={draw,rounded corners,minimum width=10mm,minimum height=5mm,align=center}]
\node[box,fill=orange!15] {$A_1\!\cdots\! A_n$} child{node[box,fill=blue!10] {$A_i\!\cdots\! A_k$}} child{node[box,fill=green!15] {$A_{k+1}\!\cdots\! A_j$}};
\node[font=\tiny,below] at (0,-1.55) {every parenthesisation splits at some $k$ --- pick cheapest $k$};
\end{tikzpicture}\end{center}
...
Example: $d=(1,3,4)$, $N=6$. Then $\mathit{dp}$ is $0,1,2,$ $1,1,2,2$
for $x=0\ldots 6$; e.g.\ $\mathit{dp}[6]=1+\min(\mathit{dp}[5],%
\mathit{dp}[3],\mathit{dp}[2])=1+1=2$ via $3+3$.
Greedy (always take largest $d_i\le x$) fails
here: it gives $4+1+1$ (3 coins).
...
\begin{enumerate}
    ...
    \item \textbf{Edit distance.} Define edit distance (insert, delete, replace cost $1$). Derive recurrence for $\mathit{ed}[i][j]$ on prefixes, state table size and filling order, and adapt to output an optimal edit sequence.
\end{enumerate}
