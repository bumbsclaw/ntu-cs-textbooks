...
\textbf{From zero: no deep learning background assumed.} We start from XOR, stack perceptrons into an MLP, and derive backpropagation from the chain rule. Every formula is preceded by a picture; activations and initialization are defined on first use.}}
\vspace{0.3em}
\section*{Learning Outcomes}
After this chapter you will be able to:
\begin{enumerate}[label=(L\arabic*)]
    \item explain why hidden layers solve XOR and state universal approximation informally;
    \item write the MLP forward pass with activations;
    \item derive the backpropagation update for one layer via the chain rule;
    \item explain initialization and vanishing gradients and train a tiny network by hand.
\end{enumerate}
